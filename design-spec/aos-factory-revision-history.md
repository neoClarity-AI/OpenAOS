...
|--------------|------------|--------|
| 2.1.3        | 2026-07-09 | Fixed a second functionality-impacting gap in the same family: `/CLAUDE.md`'s required "AGENTS.md include" section (§16.11) has always resolved to `@AGENTS.md`, and five governance-agent profiles (Chief of Staff, Review, Feedback, Security, Memory) cite an "AGENTS.md removal prohibition" as their retirement-refusal enforcement text, but no `/AGENTS.md` was ever defined as a workspace-governing file (§1.6, §4.1), scoped into plugin packaging (§28.2, runbook §36.3), or provisioned by build-aos (§9.3, §12) — a dangling include and unenforceable prohibition in every generated instance. Historical precedent (`plugin/aos-factory/templates/AGENTS.md`, commit `df581c4`) confirms this was shipped once and lost during the plugin-path migration. Added `/AGENTS.md` alongside `/aos-router.md`/`/CLAUDE.md` throughout §1.6, §4.1, §9.3, §12, §28.2, and runbook §36.3 (steps 3.5, 8); added new file_type `workspace_agent_instructions` (§15.4, distinct from the per-agent `agent_instruction`); extended §16.11 (renamed "Workspace-Root CLAUDE.md and AGENTS.md Schema") to define AGENTS.md's required sections (agent-removal governance protection, instance routing). Scoped plugin-only, mirroring the existing aos-router.md/CLAUDE.md pattern — `/aos-factory/` framework generation (§35.2) is unaffected. Fixed directly (not via a full §36.1 DRR cycle, per user decision) |
