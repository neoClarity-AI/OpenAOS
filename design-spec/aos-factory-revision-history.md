...
|--------------|------------|--------|
| 2.2.2        | 2026-07-11 | §36.1 Design Readiness Review cycle against spec_version 2.2.1 — all 19 §34 checklist items re-verified and re-marked Done; §33 safety-related governance rules confirmed satisfied; one functionality-impacting inconsistency found and resolved: the runbook's `spec_version` frontmatter (2.1.1) had drifted behind the design spec and revision history (2.2.1), so `spec_version` stamped on generated files and synced to the plugin version (§14.1, §28.2, §36.3) would have been wrong. All three canonical files reconciled to a single shared version. |
| 2.2.1        | 2026-07-11 | Rhythmic Workflow Scheduling (§9.3 step 10, §12.1) tightened: each Cowork Scheduled Task's instructions must contain exactly one line pointing to its associated workflow file, as an `@`-path reference against the instance root (e.g. `@/[aos-name]/workflows/daily-startup-workflow.md`), and nothing else — keeping the workflow file as the single source of truth for the run's behavior. `build-aos.md` (§12.1) and its packaged `SKILL.md` mirror updated to match. |
| 2.2.0        | 2026-07-11 | Retired the Quarterly Review Workflow: its goal-alignment question ("Is the whole system still aimed at the right goals?"), the advertising-check re-raise point, and retirement-signal surfacing are folded into the Monthly Review Workflow (§17.4), decided via user direction to keep monthly as the single structural/goal-alignment cadence rather than maintain two. Sections 17.6–17.9 renumbered to 17.5–17.8; every cross-reference to the old numbering (main spec, all agent profiles/interviews, `agent-catalog.yaml`, `build-aos.md`, its `SKILL.md` mirror, README) updated to match. The Feedback Agent's now-moot `candidate-presentation` cadence-choice question (monthly+quarterly vs. quarterly-only) was removed from its interview script. Added a new AOS setup step (§9.3 step 10, §12.1 `Rhythmic Workflow Scheduling`): `build-aos.md` now offers, per cadence-driven workflow (daily startup, end-of-day shutdown, weekly review, monthly review), to create a Cowork Scheduled Task invoking that workflow on its cadence — opt-in per workflow, `Proceed`-gated, logged to `/logs/change-log.md`; event-triggered workflows are never scheduled. `build-aos.md` (§12.1) and its packaged `SKILL.md` mirror updated to match. |
